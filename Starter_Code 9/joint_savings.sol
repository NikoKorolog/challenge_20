...
/* Key Points - Solidity is an Object Oriented Programming Language
THIS CODE HERE CREATES A NEW MESSAGE. The pragma line refers to the version of Solidity that we're using. 
For this module, using version 5, where the code line indicates as 0.5.0. At the end of each line of code we use a semicolon;
We specify a smart contract by using the 'contract' keyword and a set of braces ({}) to define the stard and end of the contract's code.
Solidity is a programming language that's statically typed. That means that we need to specify a data type for each solidity variable.
e.g. address is a variable type
